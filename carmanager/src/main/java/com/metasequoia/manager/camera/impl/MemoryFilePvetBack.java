...
public class MemoryFilePvetBack extends BaseMemoryFile {
    private static volatile com.metasequoia.manager.camera.MemoryFile sMemoryFile;

    public MemoryFilePvetBack() {
        super();
    }

    public static com.metasequoia.manager.camera.MemoryFile getInstance() {
        if (sMemoryFile == null) {
            synchronized (MemoryFilePvetBack.class) {
                if (sMemoryFile == null) {
                    sMemoryFile = new MemoryFilePvetBack();
                }
            }
        }
        return sMemoryFile;
    ...
    protected HandlerThread getHandlerThread() {
        return new HandlerThread("back_thread");
    }
    ...
    protected void setMemorySharedFD(FileDescriptor fd) {
        try {
            ...
            Method method = clazz.getDeclaredMethod("setShareBackFD", FileDescriptor.class);
